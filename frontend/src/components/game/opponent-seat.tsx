...
export function OpponentSeat({
  ...
  return (
    <div
      className={cn(
        "opponent-seat relative flex flex-col gap-2 rounded-2xl p-2.5 sm:p-3 transition-all select-none",
        "bg-[#051913]/90 backdrop-blur-md border",
        isTurn
          ? "border-gold-400/80 shadow-[0_0_24px_rgba(201,165,60,0.4)] ring-1 ring-gold-400/50"
          : "border-forest-500/30 hover:border-gold-500/40",
    ...
    >
      ...
      <div className="flex items-center gap-2.5 min-w-0">
        <div className="relative shrink-0">
          <span
            className={cn(
              ...
              compact ? "size-8 text-xs" : "size-9 sm:size-10 text-sm sm:text-base",
              isTurn
        ...
        <div className="flex min-w-0 flex-1 flex-col leading-tight">
          <div className="flex items-center gap-1.5 truncate">
            ...
            {isBot && (
              <span
                className="inline-flex shrink-0 items-center gap-1 rounded px-1.5 py-0.5 text-[10px] font-semibold bg-forest-900/80 text-forest-300 border border-forest-500/30"
                title={`Bot (${botDifficultyLabel(player.botDifficulty)})`}
              >
                <Bot className="size-3 text-forest-300" aria-hidden />
                <span>AI</span>
              </span>
            )}
          ...
          {isTurn && isAlive ? (
            <div className="flex items-center gap-1.5 text-[11px] font-medium text-gold-300/90 mt-0.5">
              {isBot ? (
                <>
                ...
                <span className="text-gold-400 font-semibold tracking-wide uppercase text-[10px]">
                  Their Turn
                </span>
              )}
            </div>
      ...
      <div className="flex items-center justify-between gap-2 border-t border-forest-500/20 pt-2 mt-0.5">
        {/* Coins Pill */}
        <div className="flex items-center gap-1.5 rounded-md bg-gold-500/10 px-2 py-0.5 border border-gold-500/25">
          <Coins className="size-3.5 text-gold-400 shrink-0" aria-hidden />
          <span className="font-mono text-xs sm:text-sm font-bold text-gold-300">{player.coins}</span>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {Array.from({ length: revealedCount }).map((_, i) => (
            <div
              key={`lost-${i}`}
              className="relative flex h-7 w-5 items-center justify-center rounded-xs sm:rounded-[3px] border border-crimson-500/30 bg-crimson-950/40 opacity-50"
              title="Lost Card"
            >
              <span className="text-[8px] text-crimson-400">✕</span>
            </div>
          ))}
        </div>
      </div>
