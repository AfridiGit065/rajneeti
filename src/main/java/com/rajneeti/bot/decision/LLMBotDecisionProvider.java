...
public class LLMBotDecisionProvider implements BotDecisionProvider {
    ...
    public BotDecision decide(BotDecisionContext context) {
        ...
        for (int attempt = 0; attempt < attempts; attempt++) {
            ...
            try {
                ...
                String decisionJson = extractDecisionJson(raw);
                BotDecision decision = decisionJson == null ? null
                        : parseAndValidate(context, decisionJson);
    ...
    private String extractDecisionJson(String raw) {
        try {
            ...
            log.debug("AI returned an unserializable envelope for {}: {}",
                    aiProperties.getProvider(), ex.getMessage());
            return null;
        }
    }
